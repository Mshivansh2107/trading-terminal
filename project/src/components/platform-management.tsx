...
import { platformsAtom, addPlatformAtom, updatePlatformAtom, deletePlatformAtom } from '../store/data';
...
import { PlatformEntity } from '../types';
...
const PlatformManagement = () => {
  ...
  const [, addPlatform] = useAtom(addPlatformAtom);
  const [, updatePlatform] = useAtom(updatePlatformAtom);
  const [, deletePlatform] = useAtom(deletePlatformAtom);
  
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [currentPlatform, setCurrentPlatform] = useState<PlatformEntity | null>(null);
  ...
  const handleOpenEditDialog = (platform: PlatformEntity) => {
  ...
  const handleOpenDeleteDialog = (platform: PlatformEntity) => {
    setCurrentPlatform(platform);
  ...
  const handleUpdatePlatform = async () => {
    if (!formData.name || !currentPlatform) return;
            ...
            <tbody>
              {platforms.map((platform) => (
                <tr key={platform.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">{platform.description || '-'}</td>
                  <td className="py-3 px-4 text-center">
                    <span className={`inline-block rounded-full px-2 py-1 text-xs ${
                      platform.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddPlatform} disabled={!formData.name || isSubmitting}>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdatePlatform} disabled={!formData.name || isSubmitting}>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <p className="font-medium mt-2">{currentPlatform?.name}</p>
          </div>
          <DialogFooter>
            ...
            >
              {isSubmitting ? 'Deleting...' : 'Delete Platform'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
