...
import { purchasesAtom, addPurchaseAtom, updatePurchaseAtom, deletePurchaseAtom, banksAtom, platformsAtom, refreshDataAtom, beneficiariesAtom } from '../store/data';
...
import { BeneficiarySelector } from '../components/ui/beneficiary-selector';
...
const Purchase = () => {
  ...
  const columns = useMemo(() => [
    ...
    { 
      key: 'name', 
      label: 'Name',
      formatter: (value: string, row: PurchaseEntry) => {
        const beneficiary = beneficiaries.find(b => b.id === row.beneficiaryId);
        return beneficiary ? `${beneficiary.name} (CRM)` : value;
      }
    },
    { 
      key: 'createdAt', 
